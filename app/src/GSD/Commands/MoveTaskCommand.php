...
class MoveTaskCommand extends CommandBase {
	...
	public function fire()
	{
		...
		if (is_null($taskNo)) {
			$this->abort('You must locate a task');
		}

		// Get dest list
		$destName = $this->option('dest');
		if (is_null($destName)) {
			$destName = $this->askForListId(true, true, false, 'Select destination list:');
			if (is_null($destName)) {
				$this->abort('Destination cannot be null');
			}
		}
	...
	protected function getOptions()
	{
		return array_merge(parent::getOptions(), array(
			array('dest', 'd', InputOption::VALUE_REQUIRED, 'Destination list name.', null),
			array('force', 'f', InputOption::VALUE_NONE, 'Force the move, no prompting.')
		));
	}
