...
class HelpersTest extends TestCase
{
	 ...
	 * @expetedExceptionMessage Must have at least one choice
	 */
	public function testPickFromListEmptyArrayThrowsException()
	{
		$command = Mockery::mock('Illuminate\Console\Command');

		pick_from_list($command, 'mango', array());
	}

	/**
	 * @expectedException InvalidArgumentException
	 * @expetedExceptionMessage Invalid value, default=$default
	 */
	public function testPickFromListBadDefaultThrowsException()
	{
		$command = Mockery::mock('Illuminate\Console\Command');

		pick_from_list($command, 'mango', array('option 1'), -1);
	}
	 ...
	 * @expetedExceptionMessage Invalid value, default=$default
	 */
	public function testPickFromListBadDefaultThrowsException2()
	{
		$command = Mockery::mock('Illuminate\Console\Command');

		pick_from_list($command, 'mango', array('option 1'), 2);
	}
	...
	public function testPickFromListWorksExample2()
	{
		...
		$choice = pick_from_list($command, 'title', array('option1', 'opt2'));
	...
	public function testAllNull()
	{
		...
		$this->assertFalse(all_null('string', 12));
		$this->assertFalse(all_null('string', 12, null));
