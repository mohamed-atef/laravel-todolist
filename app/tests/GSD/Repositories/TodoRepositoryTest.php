...
class TodoRepositoryTest extends TestCase
{
	...
	{
		$path = __DIR__."/data/";
		$ext = Config::get('todo.gsd.extension');
		$files = array_merge(glob($path.'*'.$ext), glob($path.'archived/*'.$ext));
	...
	{
		$this->assertTrue($this->repo->exists('test1'));
		$this->assertFalse($this->repo->exists('mango'));
	}
	...
	public function testExistsArchived()
	{
		...
		$list = $this->repo->load('test1');
		$list->set('archived', true);
		$list->set('id', 'test-archived-exist');
		$list->save();

		// test exists
		$this->assertTrue($this->repo->exists('test-archived-exist', true));
		$this->assertFalse($this->repo->exists('mango', true));
	}
	 ...
	 * @expectedExceptionMessage List with id=$id, archived=$archived not found
	 */
	public function testLoadMissingThrowsException()
	{
		$this->repo->load('mango');
	}

	public function testLodedComponents()
	{
		$list = $this->repo->load('test1');
	...
	public function testSaves()
	{
		...
		$list->set('id', 'test-two');
		$list->set('archived', true);
		$list->save();

		$file1 = trim(file_get_contents(__DIR__.'/data/test2.txt'));
		$file2 = trim(file_get_contents(__DIR__.'/data/archived/test-two.txt'));
		$this->assertEquals($file1, $file2);
	}
}
